...
function FieldListItemFactory(field, isTemplate = false) {
  console.log(field.type)
  switch (field.type) {
    case 'short-text':
      return <Input name={field.title} value={field.value} disabled={isTemplate} />;
    case 'long-text':
      return <TextArea name={field.title} value={field.value} disabled={isTemplate} />;
    case 'radio':
      return ['yes', 'no'].map((value) => <Radio name={field.title} value={field.value} label={value} disabled={isTemplate} />);
    case 'select':

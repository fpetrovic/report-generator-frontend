...
export async function loader() {
  const results = await reportApi().getAll();
...
function ReportIndex() {
  ...
  const reportTemplateTableRowElements = reports.map((report, index) => (
    <Table.Row key={index} style={{ border: '1px solid grey', margin: '20px' }}>
      ...
      <Table.HeaderCell>{report.createdAt}</Table.HeaderCell>
      <Table.HeaderCell>Does not work</Table.HeaderCell>
      <Table.HeaderCell>Does not work</Table.HeaderCell>
  ...
  return (
    <div>
      <h1>Reports</h1>
      <Link to="/reports/new" style={{ margin: '0px', 'margin-left': '10px' }}>Add New Report</Link>
      <Table celled>
        <Table.Header>
          <Table.Row>
